...
class InjectionSecurityValidator:
    ...
    def validate_injection_request(
        self, file_path: Path, injection_points: List["InjectionPoint"], config: "InjectionConfig"
    ) -> SecurityResult:
    ...
    def _validate_file_security(self, file_path: Path) -> SecurityResult:
        ...
        try:
            ...
            if dangerous_patterns:
                warnings.extend(
                    [f"Dangerous pattern found: {pattern}" for pattern in dangerous_patterns]
                )
    ...
    def _validate_injection_point_security(
        self, point: "InjectionPoint", config: "InjectionConfig"
    ) -> SecurityResult:
    ...
    def _calculate_risk_level(
        self, errors: List[str], warnings: List[str], points: List["InjectionPoint"]
    ) -> str:
    ...
    def _analyze_node_security(self, node: ast.AST) -> List[Dict[str, str]]:
        ...
        if isinstance(node, ast.Call):
            if isinstance(node.func, ast.Name) and node.func.id in self.critical_functions:
                risks.append(
                    {"severity": "medium", "description": f"Critical function call: {node.func.id}"}
                )
