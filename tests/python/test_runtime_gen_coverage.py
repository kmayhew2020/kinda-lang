...
class TestRuntimeGenerationCoverage:
    ...
    def test_generate_runtime_missing_def_in_body(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mock_constructs = {
                "bad_construct": {
                    "body": "# This is just a comment, no function definition"
                }
            }
            ...
            with patch('kinda.langs.python.runtime_gen.KindaConstructs', mock_constructs):
                with patch('builtins.print') as mock_print:
                    ...
                    assert any("No 'def' found in body for key: bad_construct, skipping env assignment" in call for call in calls)
    ...
    def test_generate_runtime_body_field_with_def_extraction(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mock_constructs = {
                "body_function": {
                    "body": "def extracted_func_name(param):\n    return param * 2"
                }
            }
