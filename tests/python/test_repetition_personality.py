            ...
            with tempfile.NamedTemporaryFile(mode="w", suffix=".knda", delete=False) as f:
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        eventually_line = [line for line in output_lines if line.startswith("EVENTUALLY:")]
...
"""
        ...
        assert eventually_range >= 3, f"Chaotic eventually_until should show variability: {eventually_range}"
...
"""
        ...
        assert 30 <= repeat_mean <= 50, f"Cautious kinda_repeat mean should be reasonable: {repeat_mean:.1f}"
        ...
        assert 25 <= eventually_mean <= 35, f"Cautious eventually_until should be reliable: {eventually_mean:.1f}"
...
"""
        ...
        assert unique_results >= 2, f"Different chaos levels should produce different results: {chaos_results}"
        ...
        if 1 in chaos_results and 10 in chaos_results:
            ...
            assert level_1_deviation <= level_10_deviation + 5, f"Chaos level 1 should be more predictable: L1={level_1_result}, L10={level_10_result}"
...
"""
            ...
            with tempfile.NamedTemporaryFile(mode="w", suffix=".knda", delete=False) as f:
                ...
                try:
                    ...
                    assert result.returncode == 0, f"Extreme chaos level {chaos_level} failed: {result.stderr}"
                    ...
                    if eventually_line:
                        ...
                        assert 10 <= eventually_count <= 105, f"Extreme chaos should still terminate reasonably: {eventually_count}"
...
"""
            ...
            if personality in ["reliable", "cautious"]:
                ...
                assert stdev_result <= expected_stdev + tolerance, f"{personality}: Variance too high: {stdev_result:.2f} vs {expected_stdev:.2f} ± {tolerance:.2f}"
            elif personality == "chaotic":
                # Chaotic should have higher variance
                assert stdev_result >= expected_stdev - tolerance, f"{personality}: Variance too low for chaotic: {stdev_result:.2f} vs {expected_stdev:.2f} ± {tolerance:.2f}"
...
"""
            ...
            assert all(tp >= 20 for tp in termination_points), f"{personality}: Should meet base condition"
            
            # Higher confidence should terminate closer to threshold
            if personality == "reliable":
                # Reliable should terminate quickly after condition is met
                assert mean_termination <= 30, f"Reliable should terminate quickly: {mean_termination:.1f}"
            elif personality == "chaotic":
                # Chaotic might terminate later due to lower confidence
                assert mean_termination >= 20, f"Chaotic should still meet condition: {mean_termination:.1f}"
            
            # Should not run excessively long regardless of confidence
            assert max(termination_points) <= 80, f"{personality}: Should not run excessively long: max={max(termination_points)}"
...
"""
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".knda", delete=False) as f:
            ...
            try:
                ...
                assert result.returncode == 0, f"Personality transition test failed: {result.stderr}"
                ...
                assert reliable_count != chaotic_count or abs(reliable_count - chaotic_count) <= 5, "Personality change should affect behavior"
...
"""
            ...
            if len(set(results)) == 1:
                ...
                assert result_range <= 1, f"{personality}: Same seed should give consistent results: {results}"
