            ...
            with patch('kinda.cli.get_transformer', return_value=mock_transformer):
                with patch('runpy.run_path') as mock_run_path:
                    ...
                    with patch('builtins.print') as mock_print:
                        ...
                        assert any("Your code transformed fine but crashed during execution" in call for call in calls)
        finally:
            os.unlink(temp_path)
            ...
            with patch('kinda.cli.detect_language', return_value='javascript'):
                ...
                with patch('kinda.cli.get_transformer', return_value=mock_transformer):
                    with patch('builtins.print') as mock_print:
                        ...
                        assert result == 1
                        calls = [call.args[0] for call in mock_print.call_args_list]
                        assert any("I can transform javascript but can't run it" in call for call in calls)
        finally:
            os.unlink(temp_path)
    ...
    def test_interpret_command_file_suggestions(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('builtins.print') as mock_print:
                ...
                assert any("These .knda files are available for interpretation:" in call for call in calls)
    ...
    def test_interpret_command_file_validation_fails(self):
        ...
        try:
            with patch('builtins.print') as mock_print:
                ...
                assert result == 1
                calls = [call.args[0] for call in mock_print.call_args_list]
                assert any("File validation failed - cannot interpret this file" in call for call in calls)
        finally:
            os.unlink(temp_path)
