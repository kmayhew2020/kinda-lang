...
class TestSeedCLIIntegration(unittest.TestCase):
    ...
    def test_setup_personality_invalid_env_seed(self):
        ...
        mock_print.assert_any_call(
            "[?] Invalid KINDA_SEED value 'not_a_number' - ignoring environment variable"
        )
...
~sorta print("Result:", result)"""
    ...
    def test_seed_validation_bounds_checking(self):
        ...
        with patch("kinda.cli.safe_print") as mock_print:
            ...
            mock_print.assert_any_call(
                f"[?] Seed value {MAX_SEED + 1} is outside safe range ({MIN_SEED} to {MAX_SEED})"
            )
        ...
        with patch("kinda.cli.safe_print") as mock_print:
            ...
            mock_print.assert_any_call(
                f"[?] Seed value {MIN_SEED - 1} is outside safe range ({MIN_SEED} to {MAX_SEED})"
            )
    ...
    def test_seed_validation_integration(self):
        ...
        mock_print.assert_any_call(
            f"[?] Seed value {MAX_SEED + 1000} is outside safe range ({-(2**31)} to {MAX_SEED})"
        )
