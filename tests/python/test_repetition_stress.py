...
"""
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".knda", delete=False) as f:
            ...
            try:
                ...
                assert count <= 1400, f"Chaotic variance shouldn't exceed reasonable bounds, got {count}"
...
"""
            ...
            with tempfile.NamedTemporaryFile(mode="w", suffix=".knda", delete=False) as f:
                ...
                try:
                    ...
                    assert result.returncode == 0, f"Script failed for input {input_val}: {result.stderr}"
                    ...
                    count = int(result_line[0].split(":")[1])
                    assert count >= expected_min, f"For input {input_val}, expected >= {expected_min}, got {count}"
...
"""
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".knda", delete=False) as f:
            ...
            try:
                ...
                # With reliable personality (95% confidence), should terminate shortly after condition becomes true
                assert count > 200, f"Should terminate after condition becomes true, got {count}"
                assert count < 300, f"Should not run too long beyond condition, got {count}"
...
"""
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".knda", delete=False) as f:
            ...
            try:
                ...
                assert execution_time < 25, f"Should timeout or break quickly: {execution_time:.2f}s"
...
"""
        ...
        # With reliable personality (95% confidence), most should terminate shortly after threshold
        avg_result = sum(results) / len(results)
        assert 50 <= avg_result <= 65, f"Average result should be close to threshold, got {avg_result:.1f}"
        
        # Variability should be reasonable
        max_result = max(results)
        min_result = min(results)
        range_span = max_result - min_result
        assert range_span <= 30, f"Range should be reasonable with reliable personality, got {range_span}"
