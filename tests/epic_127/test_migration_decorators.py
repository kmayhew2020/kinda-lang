...
class TestGradualKindaDecorator:
    ...
    def test_gradual_kinda_probability_validation(self):
        ...
        # Test valid probability - this should work without error
        @gradual_kinda(probability=0.3)
        def valid_function():
            return "valid"

        # Verify the function was decorated and can be called
        assert valid_function() == "valid"

        # Test invalid probability - this should raise ValueError
    ...
    def test_gradual_kinda_with_different_probabilities(self):
        ...
        for prob in probabilities:
            # Each decorator should work without error
            @gradual_kinda(probability=prob)
            def test_func():
                return f"prob_{prob}"

            # Function should be callable and return expected result
            result = test_func()
            assert result == f"prob_{prob}"
...
class TestKindaSafeDecorator:
    ...
    def test_kinda_safe_basic_functionality(self):
        ...
        # Function should work normally when no error occurs
