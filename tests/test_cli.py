...
class TestCLICommands:
    ...
    def test_examples_command(self, capsys):
        ...
        assert "60% conditional execution" in output
        assert "Sometimes Demo" in output
        assert "Maybe Math" in output
...
class TestCLIIntegration:
    ...
    def test_examples_command_integration(self):
        ...
        assert "60% conditional execution" in result.stdout
    ...
    def test_transform_nonexistent_file(self):
        ...
        result = subprocess.run(
            ["kinda", "transform", "does_not_exist.knda"], capture_output=True, text=True
        )
        assert result.returncode != 0
    ...
    def test_run_with_actual_example(self):
        ...
        if example_file.exists():
            result = subprocess.run(
                ["kinda", "transform", str(example_file)], capture_output=True, text=True
            )
