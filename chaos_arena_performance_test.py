...
def analyze_file_complexity(file_path):
    ...
    metrics = {
        ...
        "total_constructs": (
            ...
            + content.count("~sorta")
            + content.count("~maybe")
            + content.count("~sometimes")
            + content.count("~ish")
            + content.count("~welp")
            + content.count("kinda_binary")
            + content.count("~=")
        ),
...
def run_chaos_arena_performance_test():
    ...
    for file_path in existing_files:
        ...
        if complexity:
            ...
            constructs = []
            if complexity["kinda_constructs"]:
                constructs.append(f"~kinda({complexity['kinda_constructs']})")
            if complexity["sorta_constructs"]:
                constructs.append(f"~sorta({complexity['sorta_constructs']})")
            if complexity["maybe_constructs"]:
                constructs.append(f"~maybe({complexity['maybe_constructs']})")
            if complexity["sometimes_constructs"]:
                constructs.append(f"~sometimes({complexity['sometimes_constructs']})")
            if complexity["ish_constructs"]:
                constructs.append(f"~ish({complexity['ish_constructs']})")
            if complexity["welp_constructs"]:
                constructs.append(f"~welp({complexity['welp_constructs']})")
            if complexity["binary_constructs"]:
                constructs.append(f"binary({complexity['binary_constructs']})")
            if complexity["fuzzy_assigns"]:
                constructs.append(f"~=({complexity['fuzzy_assigns']})")
        ...
        if perf_result:
            ...
            results[file_path.name] = {"complexity": complexity, "performance": perf_result}
        else:
    ...
    if results:
        ...
        for i, (name, data) in enumerate(sorted_results, 1):
            ...
            print(f"  {i}. {name}")
            print(
                f"     Complexity: {complexity['complexity_score']:.3f} | "
        ...
        all_complexities = [
            r["complexity"]["complexity_score"] for r in results.values() if r["complexity"]
        ]
        ...
        print(
            f"   Most time-consuming: {max_time_file[0]} ({max_time_file[1]['performance']['mean']:.1f}ms)"
        )
        print(
            f"   Most complex: {max_complexity_file[0]} (score: {max_complexity_file[1]['complexity']['complexity_score']:.3f})"
        )
